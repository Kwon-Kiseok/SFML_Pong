...
int main()
{
    ...
    Ball ball((bat.GetGlobalBounds().left + bat.GetGlobalBounds().width/2), bat.GetGlobalBounds().top - 20.f);
    ...
    while (window.isOpen())
    {
        ...
        if (isGameStart)
        {
            ...
            if ((ballBound.top + ballBound.height > windowSize.y) && (prevBallBound.top + prevBallBound.height < windowSize.y))
            {
                //ball.ReboundBottom();
                isGameStart = false;
                ball.SetXY((bat.GetGlobalBounds().left + bat.GetGlobalBounds().width / 2), bat.GetGlobalBounds().top - 20.f);
